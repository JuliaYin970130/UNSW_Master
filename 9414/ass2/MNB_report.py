...
# articles - b
MNB_count_vec_b = CountVectorizer(preprocessor=preprocessing, max_features=1000, lowercase=False,token_pattern='[\w#_@$%]{2,}')
...
index = np.arange(3)
rects1 = plt.bar(index, marco_a_, bar_width, color='y', label='with whole vocabulary',alpha = 0.5)
rects2 = plt.bar(index + bar_width, marco_b_, bar_width, color='g', label='with most frequent 1000 words',alpha = 0.5)

plt.xticks(index + (bar_width/2), metrics)
plt.ylim(ymax=1.2)
plt.title('MNB_Macro_summaries')
#plt.title('MNB_Marco_summaries')
plt.xlabel('metrics')
plt.ylabel('score')
...
index = np.arange(4)
rects1 = plt.bar(index, mirco_a_, bar_width, color='y', label='with whole vocabulary',alpha = 0.5)
rects2 = plt.bar(index + bar_width, mirco_b_, bar_width, color='g', label='with most frequent 1000 words',alpha = 0.5)

plt.xticks(index + (bar_width/2), metrics)
plt.ylim(ymax=1.2)
plt.title('MNB_Micro_summaries')
#plt.title('MNB_Mirco_summaries')
plt.xlabel('metrics')
plt.ylabel('score')
plt.legend(loc='upper right', fancybox=True)
