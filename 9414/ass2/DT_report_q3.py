...
###################### model ###############################
min_samples = round(len(data_X_train) / 100)

## a
DT_count_vec_a = CountVectorizer(preprocessor=preprocessing, max_features=1000, lowercase=False, token_pattern='[\w#_@$%]{2,}')
X_train_bag_of_words_a = DT_count_vec_a.fit_transform(data_X_train)
X_test_bag_of_words_a = DT_count_vec_a.transform(data_X_test)

DT_clf_a = tree.DecisionTreeClassifier(min_samples_leaf=min_samples, criterion='entropy', random_state=0)
DT_model_a = DT_clf_a.fit(X_train_bag_of_words_a, data_Y_train)
acc_a, micro_a, macro_a = predict_and_test(DT_model_a,X_test_bag_of_words_a,data_Y_test)

# 加stop word 和 PorterStemmer
## b
DT_count_vec_b = CountVectorizer(preprocessor=preprocessing_2, max_features=1000, stop_words='english', lowercase=False, token_pattern='[\w#_@$%]{2,}')
X_train_bag_of_words_b = DT_count_vec_b.fit_transform(data_X_train)
X_test_bag_of_words_b = DT_count_vec_b.transform(data_X_test)


DT_clf_b = tree.DecisionTreeClassifier(min_samples_leaf=min_samples, criterion='entropy', random_state=0)
DT_model_b = DT_clf_b.fit(X_train_bag_of_words_b, data_Y_train)
acc_b, micro_b, macro_b = predict_and_test(DT_model_b,X_test_bag_of_words_b,data_Y_test)
...
bar_width = 0.35
index = np.arange(4)
rects1 = plt.bar(index, mirco_a_, bar_width, color='y', label='model a',alpha=0.5)
rects2 = plt.bar(index + bar_width, mirco_b_, bar_width, color='g', label='model b', alpha=0.5)
plt.xticks(index + (bar_width/2), metrics)
plt.title('DT_Micro_summaries')
plt.xlabel('metrics')
plt.ylabel('score')
plt.ylim(ymax=1)
...
bar_width = 0.35
index = np.arange(3)
rects1 = plt.bar(index, marco_a_, bar_width, color='y', label='model a',alpha=0.5)
rects2 = plt.bar(index + bar_width, marco_b_, bar_width, color='g', label='model b', alpha=0.5)
plt.xticks(index + (bar_width/2), metrics)
plt.title('DT_Macro_summaries')
plt.xlabel('metrics')
plt.ylabel('score')
plt.ylim(ymax=1)
plt.legend(loc='upper right', fancybox=True)
