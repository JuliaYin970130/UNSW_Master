...
def initial_env(input):
    ...
    for line in input:
        ...
        if words[0] == 'constraint':
            ...
            # print(condition)
            # duration1 = durations[words[1]]  # duration for the first task
            # duration2 = durations[words[3]]  # duration for the second task
            # dealing with different key words

            if words[2] == 'before':
                constraints.append(Constraint([words[1], words[3]],
                                              lambda a, b: a[1] < b[0],
                                              "{}<{}".format(words[1], words[3])))
            elif words[2] == 'after':
                constraints.append(Constraint([words[1], words[3]],
                                              lambda a, b: a[0] > b[1],
                                              "{}>{}".format(words[1], words[3])))
                ...
                constraints.append(Constraint([words[1], words[3]],
                                              lambda a, b: a[1] + 1 == b[0],
                                              "{}+{}={}".format(words[1], 1, words[3])))
            elif words[2] == 'overlaps':
                constraints.append(Constraint([words[1], words[3]],
                                              lambda a, b:  b[0] > a[0] and b[0] <= a[1] and b[1] > a[1],
                                                  "{}>{} and {}<={} and {}>{}".format(words[3], words[1],words[3], words[1], words[3],words[1])))
                # (t1[0] < t2[0] and t1[1] >= t2[0]) and t1[1] < t2[1]

            elif words[2] == 'during':
                constraints.append(Constraint([words[1], words[3]],
                                              lambda a, b: a[0] > b[0] and a[1] < b[1],
                                              "{}>{} and {}<{}".format(words[1], words[3], words[1],words[3])))
            elif words[2] == 'equals':
                constraints.append(Constraint([words[1], words[3]],
                                              lambda a, b: a[0] == b[0] and a[1] == b[1],
                                              "{}={} and {}={}".format(words[1], words[3], words[1],words[3])))
